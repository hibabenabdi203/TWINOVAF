...
// Prevent accidental page reloads: untyped buttons default to submit.
document.querySelectorAll('button:not([type])').forEach((btn) => {
  btn.type = 'button';
});
// Prevent native form submits from reloading the SPA.
...
verifierConnexion();
const lastPage = localStorage.getItem('twinova_active_page');
if (lastPage && lastPage !== 'dashboard') {
  setTimeout(() => navigate(lastPage), 0);
}
// ── DATE ─────────────────────────────────────────────
function updateDate() {
...
function navigate(pageId) {
  ...
  localStorage.setItem('twinova_active_page', pageId);
